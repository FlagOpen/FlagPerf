...
def resultshow():
    ...
    path = grandparent_path + "/tasks/" + config_dict[
        "vendor"] + "/" + config_dict["engine"] + "/" + "task.yaml"
    gpu_config = read_yaml_file(path)

    infer_api = config_dict["engine"]

    log_path = "./log" + "/" + str(infer_api)
    ...
    tflops = gpu_config["GPU_FP16TC"]
    MFU = (token_per_s * 2 * parameters) / (tflops * 10e12 *
                                            config_dict["nproc_per_node"])
    return ttft, newly_tokens, total_tokens, duration, token_per_s, rougeone, rougetwo, rougeL, MFU
