...
def main(config, case_config):
    ...
    print("Test Correctness with 1k-times smaller operation"
    ...
    for i in range(100):
        a = torch.randn(Melements, 1024, dtype=dtype[config.dataformat])
    ...
    op2flops = lambda x: x * 3 * Melements * 1024 * 1024

    perf_result = cal_perf(cputime, kerneltime, op2flops,
                           case_config.SPECTFLOPS)
    print_result(config, "softmax", *perf_result, mape, mape_std,
