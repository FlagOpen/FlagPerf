...
def main(config, case_config):
    ...
    for i in range(100):
        ...
        c = torch.randn(m//16, k//16, dtype=dtype[config.dataformat])

        a_fp64 = a.to(torch.float64)
        b_fp64 = b.to(torch.float64)
        c_fp64 = c.to(torch.float64)
        r_fp64 = torch.addmm(c_fp64, a_fp64, b_fp64)

        a = a.to(0)
        b = b.to(0)
        c = c.to(0)
        r_device = torch.addmm(c, a, b).cpu()
    ...
    print_result(config, "addmm", *perf_result, mape, mape_std,
