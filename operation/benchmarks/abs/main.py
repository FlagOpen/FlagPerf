...
def main(config, case_config):
    ...
    print("Setting up environment for abs operator"
          )  # Environment setup for abs operator
    ...
    for i in range(100):
        # Main computation block for abs operator
        ...
        r_fp64 = torch.abs(a_fp64 + b_fp64)

        a = a.to(0)
        b = b.to(0)
        r_device = torch.abs(a + b).cpu()
    ...
    latency_nowarm, latency_warm, cputime, kerneltime = do_test(
        torch.abs, (a, b), host_device_sync, config, case_config)
