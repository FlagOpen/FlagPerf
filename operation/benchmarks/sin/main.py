...
def main(config, case_config):
    ...
    print("Setting up environment for sin operator"
          )  # Environment setup for sin operator
    ...
    for i in range(100):
        # Main computation block for sin operator
        ...
        r_fp64 = torch.sin(a_fp64 + b_fp64)

        a = a.to(0)
        b = b.to(0)
        r_device = torch.sin(a + b).cpu()
    ...
    latency_nowarm, latency_warm, cputime, kerneltime = do_test(
        torch.sin, (a, b), host_device_sync, config, case_config)
