...
def clear_caches_cluster(clear, nnodes):
    ...
    clear_cmd = "sync && echo password | sudo -S /sbin/sysctl vm.drop_caches=3"
    timeout = 30
...
def start_monitors_in_cluster(dp_path, case_log_dir, nnodes):
    ...
    start_mon_cmd = "cd " + dp_path + " && echo password | sudo -S " + sys.executable \
...
def stop_monitors_in_cluster(dp_path, nnodes):
    ...
    ven_mon_path = os.path.join(dp_path, "docker_images", config.VENDOR,
                                config.VENDOR + "_monitor.py")
    stop_mon_cmd = "cd " + dp_path + " && echo password | sudo -S " + sys.executable \
                   + " " + ven_mon_path + " -o stop"
...
def start_tasks_in_cluster(dp_path, container_name, case_config, curr_log_path,
    ...
    start_cmd = "cd " + dp_path + " && " + sys.executable \
                + " utils/container_manager.py -o runcmdin -c " \
                + container_name + " -r \"" \
                + f"export vendor=" + getattr(config, "VENDOR") + ";" \
    ...
    CLUSTER_MGR.run_command_some_hosts_distribution_info(start_cmd, nnodes, 10800)
