...
# from config import test_conf as config

# from utils import flagperf_logger
# from utils import image_manager

VERSION = "v0.1"
# logger = flagperf_logger.FlagPerfLogger()
...
def print_welcome_msg():
    ...
    logger.log("Welcome", "\033[1;34;40m==============================================\033[0m")
    logger.log("Welcome", "\033[1;36;40m          Welcome to FlagPerf Inference!\033[0m")
    logger.log("Welcome", "\033[1;36;40m      See more at https://baai.ac.cn/ \033[0m")
    logger.log("Welcome", "\033[1;34;40m==============================================\033[0m")
...
def check_cluster_deploy_path(dp_path):
    ...
    if len(bad_hosts) != 0:
        ...
        logger.error("Check cluster deploy path " + dp_path +
                         "......[FAILED] [EXIT]")
        sys.exit(3)
    logger.info("Check flagperf deployment path: " + dp_path +
                    "...[SUCCESS]")
...
def check_test_host_config(config):
    ...
    must_para = ['FLAGPERF_LOG_PATH','FLAGPERF_LOG_PATH','VENDOR','FLAGPERF_LOG_LEVEL',
                 'HOSTS','SSH_PORT','HOSTS_PORTS', 'MASTER_PORT', 'SHM_SIZE', 'ACCE_CONTAINER_OPT',
                 'PIP_SOURCE','CLEAR_CACHES','ACCE_VISIBLE_DEVICE_ENV_NAME','CASES']
...
def prepare_docker_image_cluster(dp_path, image_mgr, framework, nnodes, config):
    ...
    image_vendor_dir = os.path.join(CURR_PATH, "docker_images/" + vendor + "/" + framework)
    image_name = image_mgr.repository + ":" + image_mgr.tag
    logger.debug("Prepare docker image in cluster. image_name=" +
...
def start_monitors_in_cluster(dp_path, case_log_dir, nnodes):
    ...
    ven_mon_path = os.path.join(dp_path, config.VENDOR, config.VENDOR + "_monitor.py")
    start_mon_cmd = "cd " + dp_path + " && " + sys.executable \
...
def start_tasks_in_cluster(dp_path, container_name, case_config,
                           count, curr_log_path, config):
    ...
    must_configs =["FLAGPERF_PATH", "FLAGPERF_LOG_PATH", 
                    "MODEL", "VENDOR", "FLAGPERF_LOG_LEVEL"]
    ...
    if (os.path.isfile(env_file)):
        start_cmd = "cd " + dp_path + " && " + sys.executable \
                ...
                + "2>&1 && " \
                + "python3 " + config.FLAGPERF_PATH + "/run_benchmarks/" \
        ...
        start_cmd = "cd " + dp_path + " && " + sys.executable \
                ...
                + f" --log_dir " + curr_log_path  + " 2>&1 | tee result/0.out.log" + "\""
...
def prepare_containers_env_cluster(dp_path, config, container_name,
...
def collect_and_merge_logs(curr_log_path, cases):
    ...
    for case in cases:
        ...
        for i in range(1, repeat + 1):
            ...
            logger.debug("Case " + case + ", round " + str(i) +
...
def get_config_from_case(case, config):
    ...
    if len(case_info) < 2:
        logger.error(
            "At least 2 terms split by \":\" should in config.CASES")
...
def get_valid_cases(config):
    ...
    for case in config.CASES:
        rets, case_config = get_config_from_case(case, config)
        if (not rets) or (not check_case_config(case, case_config, config.VENDOR)):
...
def prepare_case_config_cluster(dp_path, case_config, case):
    ...
    nnodes = case_config["nnodes"]
    case_config_dir = os.path.join(dp_path, config.VENDOR, model + "-" + framework,
                                   "config")
...
def main(config):
    ...
    logger.log("Env Info", "\033[1;36;40m======== Step 1: Check key configs. ========\033[0m")
    ...
    logger.log("Env Info", "\033[1;36;40m========= Step 2: Prepare and Run test cases. =========\033[0m")
    ...
    for case in cases:
        ...
        image_mgr = image_manager.ImageManager(
            "flagperf-inference-" + config.VENDOR + "-" + case_config["framework"],
            "t_" + VERSION)
        image_name = image_mgr.repository + ":" + image_mgr.tag
        nnodes = case_config["nnodes"]
        logger.log("Env Info", "\033[1;36;40m=== 2.1 Prepare docker image:" + image_name + " ===\033[0m")
        ...
        for count in range(1, case_config["repeat"] + 1):
            ...
            if not prepare_containers_env_cluster(dp_path, config,
    ...
    logger.info("Env Info", "\033[1;36;40m========= Step 3: Collect logs in the cluster. =========\033[0m")
    # collect_and_merge_logs(curr_log_path, cases) #TODO
...
if __name__ == '__main__':
    ...
    # fake, just for test
    config = lambda: None
    config.FLAGPERF_PATH = "/home/shihonghao/wwl_inference"
