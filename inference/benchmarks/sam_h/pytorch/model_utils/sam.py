...
class SamAttention(nn.Module):
    ...
    def _separate_heads(self, hidden_states: Tensor,
                        num_attention_heads: int) -> Tensor:
        batch, point_batch_size, n_tokens, channel = hidden_states.shape
        c_per_head = channel // num_attention_heads
        hidden_states = hidden_states.reshape(batch * point_batch_size,
                                              n_tokens, num_attention_heads,
                                              c_per_head)
        return hidden_states.transpose(1, 2)

    def _recombine_heads(self, hidden_states: Tensor,
                         point_batch_size: int) -> Tensor:
        batch, n_heads, n_tokens, c_per_head = hidden_states.shape
        hidden_states = hidden_states.transpose(1, 2)
        return hidden_states.reshape(batch // point_batch_size,
                                     point_batch_size, n_tokens,
                                     n_heads * c_per_head)
    ...
    def forward(self,
        ...
        attn = attn / math.sqrt(float(c_per_head))
...
class SamTwoWayAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        attn_out = self.cross_attn_image_to_token(query=key,
                                                  key=query,
                                                  value=queries)
        keys = keys + attn_out
...
class SamTwoWayTransformer(nn.Module):
    ...
    def forward(
        ...
        image_embeddings = image_embeddings.flatten(2).permute(0, 2,
                                                               1).unsqueeze(1)
        image_positional_embeddings = image_positional_embeddings.flatten(
            2).permute(0, 2, 1).unsqueeze(1)
        ...
        attn_out = self.final_attn_token_to_image(query=query,
                                                  key=key,
                                                  value=keys)
...
class SamMaskDecoder(nn.Module):
    ...
    def __init__(self, config: SamMaskDecoderConfig):
        ...
        self.upscale_conv1 = nn.ConvTranspose2d(self.hidden_size,
                                                self.hidden_size // 4,
                                                kernel_size=2,
                                                stride=2)
        self.upscale_conv2 = nn.ConvTranspose2d(self.hidden_size // 4,
                                                self.hidden_size // 8,
                                                kernel_size=2,
                                                stride=2)
        ...
        for _ in range(self.num_mask_tokens):
            mlps_list += [
                SamFeedForward(self.hidden_size, self.hidden_size,
                               self.hidden_size // 8, 3)
            ]
        self.output_hypernetworks_mlps = nn.ModuleList(mlps_list)

        self.iou_prediction_head = SamFeedForward(self.hidden_size,
    ...
    def forward(
        ...
        upscaled_embedding = self.activation(
            self.upscale_conv2(upscaled_embedding))
...
class SamPositionalEmbedding(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.register_buffer(
            ...
            self.scale * torch.randn((2, config.num_pos_feats)))
    ...
    def forward(self, input_coords, input_shape=None):
        ...
        return torch.cat([torch.sin(coordinates),
...
class SamMaskEmbedding(nn.Module):
    ...
    def __init__(self, config: SamPromptEncoderConfig):
        ...
        self.conv1 = nn.Conv2d(1,
                               ...
                               kernel_size=2,
                               stride=2)
        self.conv2 = nn.Conv2d(self.mask_input_channels,
                               config.mask_input_channels,
                               kernel_size=2,
                               stride=2)
        self.conv3 = nn.Conv2d(config.mask_input_channels,
                               config.hidden_size,
                               kernel_size=1)
...
class SamPromptEncoder(nn.Module):
    ...
    def _embed_points(self, points: torch.Tensor, labels: torch.Tensor,
        ...
        if pad:
            target_point_shape = (points.shape[0], points.shape[1], 1,
                                  points.shape[-1])
        ...
        point_embedding = torch.where(labels[..., None] == -1,
    ...
    def forward(
        ...
        if input_points is not None:
            ...
            point_embeddings = self._embed_points(input_points,
                                                  input_labels,
...
class SamVisionAttention(nn.Module):
    ...
    def __init__(self, config, window_size):
        ...
        input_size = ((config.image_size // config.patch_size,
                       config.image_size //
                       config.patch_size) if window_size == 0 else
    ...
    def get_rel_pos(self, q_size: int, k_size: int,
        ...
        relative_coords = (
            q_coords - k_coords) + (k_size - 1) * max(q_size / k_size, 1.0)
    ...
    def add_decomposed_rel_pos(
        ...
        relative_position_height = self.get_rel_pos(query_height, key_height,
                                                    rel_pos_h)
        relative_position_width = self.get_rel_pos(query_width, key_width,
                                                   rel_pos_w)

        batch_size, _, dim = query.shape
        reshaped_query = query.reshape(batch_size, query_height, query_width,
                                       dim)
        rel_h = torch.einsum("bhwc,hkc->bhwk", reshaped_query,
                             relative_position_height)
        rel_w = torch.einsum("bhwc,wkc->bhwk", reshaped_query,
                             relative_position_width)
        attn = attn.reshape(batch_size, query_height, query_width, key_height,
                            key_width)
        attn = attn + rel_h[:, :, :, :, None] + rel_w[:, :, :, None, :]
        attn = attn.reshape(batch_size, query_height * query_width,
    ...
    def forward(self,
        ...
        qkv = (self.qkv(hidden_states).reshape(batch_size, height * width, 3,
                                               self.num_attention_heads,
                                               -1).permute(2, 0, 3, 1, 4))
        # q, k, v with shape (batch_size * nHead, height * width, channel)
        query, key, value = qkv.reshape(3,
                                        batch_size * self.num_attention_heads,
                                        height * width, -1).unbind(0)

        attn_weights = (query * self.scale) @ key.transpose(-2, -1)

        if self.use_rel_pos:
            attn_weights = self.add_decomposed_rel_pos(attn_weights, query,
                                                       self.rel_pos_h,
                                                       self.rel_pos_w,
                                                       (height, width),
                                                       (height, width))
        ...
        attn_output = (attn_probs @ value).reshape(batch_size,
                                                   self.num_attention_heads,
                                                   height, width, -1)
        attn_output = attn_output.permute(0, 2, 3, 1,
                                          4).reshape(batch_size, height, width,
                                                     -1)
...
class SamVisionLayer(nn.Module):
    ...
    def window_unpartition(self, windows: torch.Tensor, window_size: int,
        ...
        batch_size = windows.shape[0] // (pad_height * pad_width //
                                          window_size // window_size)
        hidden_states = windows.reshape(batch_size, pad_height // window_size,
                                        pad_width // window_size, window_size,
                                        window_size, -1)
        hidden_states = (hidden_states.permute(0, 1, 3, 2, 4,
                                               5).contiguous().reshape(
                                                   batch_size, pad_height,
                                                   pad_width, -1))
    ...
    def forward(
        ...
        if self.window_size > 0:
            hidden_states = self.window_unpartition(hidden_states,
                                                    self.window_size,
                                                    padding_shape,
                                                    (height, width))
...
class SamVisionNeck(nn.Module):
    ...
    def __init__(self, config: SamVisionConfig):
        ...
        self.conv1 = nn.Conv2d(config.hidden_size,
                               config.output_channels,
                               kernel_size=1,
                               bias=False)
        self.layer_norm1 = SamLayerNorm(config.output_channels,
                                        data_format="channels_first")
        self.conv2 = nn.Conv2d(config.output_channels,
                               config.output_channels,
                               kernel_size=3,
                               padding=1,
                               bias=False)
    ...
    def __init__(self, config: SamVisionConfig):
        ...
        for i in range(config.num_hidden_layers):
            layer = SamVisionLayer(
                ...
                if i not in config.global_attn_indexes else 0,
            )
...
class SamModel(SamPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.shared_image_embedding = SamPositionalEmbedding(
