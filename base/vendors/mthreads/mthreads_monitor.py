...
class Daemon:
    ...
    def run(self):
        ...
        def gpu_mon(file):
            ...
            cmd = "paste <(mthreads-gmi -q |grep 'GPU Current Temp') <(mthreads-gmi -q |grep 'Power Draw') <(mthreads-gmi -q |grep 'Used') <(mthreads-gmi -q |grep 'Total') <(mthreads-gmi -q |grep 'Gpu') | awk '{print $5,$9,$12,$15,$18}'"
...
def main():
    ...
    subdaemon = Daemon(
        pid_fn, log_fn, err_fn, gpu_fn, log_path, verbose=1, rate=sample_rate1
    )
    if operation == "start":
        subdaemon.start()
    elif operation == "stop":
