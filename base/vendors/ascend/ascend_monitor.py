...
class Daemon:
    ...
    def run(self):
        ...
        def gpu_mon(file):
            ...
            cmd = cmd + "export LD_LIBRARY_PATH=/usr/lical/Ascend/dirver/lib64/driver/:$LD_LIBRARY_PATH" + ";"
            cmd = cmd + "paste -d' ' <(npu-smi info | grep OK | awk -F'|' '{print $4, $5}' | awk '{print $1, $2}') <(npu-smi info | grep 65536 | awk -F'|' '{print $4, $5}' | awk '{print $1, $5}')"
...
def main():
    ...
    operation = args.o
    log_path = args.l
    pid_fn = str('/tmp/npu_monitor.pid')
    log_fn = str(log_path + '/ascend_monitor.log')
    err_fn = str(log_path + '/ascend_monitor.err')
    # result for gpu
    gpu_fn = str(log_path + '/ascend_monitor.log')
