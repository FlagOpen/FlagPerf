...
def main(config, case_config, rank, world_size, local_rank):    
    ...
    matrixA = torch.randint(low=-128, high=127,size=(m, n), dtype=torch.int8).to(local_rank)
    matrixB = torch.randint(low=-128, high=127,size=(n, k), dtype=torch.int8).to(local_rank)
