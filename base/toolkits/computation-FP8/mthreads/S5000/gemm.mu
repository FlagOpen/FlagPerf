    ...
    TestMatMul(const musaStream_t& _stream, const int _device_id, const DType _dtype, const MatMulParam _param, const int _iters)
    {
        ...
        switch (dtype) {
            ...
            case DType::q8:
                ...
                break;
            case DType::f8:
                dtype_str = "float8_e4m3fn"; // only support e4m3 for now
                dtype_size = 4;// for temp float data
                break;
            default:
...
#undef FREE_D
    ...
    void ShowPerformance(float t, float gops, bool credible) {
        ...
        SHOW("Average FP8 Single Op Duration:%f us\n", t * 1.0e3);
        SHOW("[FlagPerf Result]computation-FP8=%f TFLOPS\n", gops / 1.0e3);
...
private:
    ...
    // fp8，暂时按照需要的参数进行命名
    ::musa::dnn::BatchMatMul op_f8;
    std::string f8_tmp_dtype_str = "float32";
    ::musa::dnn::Tensor amax;

    ::musa::dnn::MatMulLtParam f8_param;

    float f8_scale_a; // scale_a
    float f8_scale_b; // scale_b
    ...
    void* d_golden_amax; // golden_amax on gpu

    void* d_temp = nullptr; // The gpu_abs_max() interface is required but has no actual use
    void* d_f8_a = nullptr;
    void* d_f8_b = nullptr;
    void* d_f8_c = nullptr; // fp8 result matrix


    void* d_f32_tmp_a = nullptr;
    void* d_f32_tmp_b = nullptr;
    void* d_golden = nullptr; // temp value: matmul result of fp32 temp matrix 

    // matrix tensor
    ::musa::dnn::Tensor tensor_f8_a;
    ::musa::dnn::Tensor tensor_f8_b;
    ::musa::dnn::Tensor tensor_f8_c; // tensor of fp8 result matrix
    ::musa::dnn::Tensor tensor_f32_tmp_a;
    ::musa::dnn::Tensor tensor_f32_tmp_b;
    ::musa::dnn::Tensor tensor_golden; // d_golden 


    // scale tensor
    ::musa::dnn::Tensor tensor_temp; // The param_fp8.SetScale() interface is required but has no actual use
    ::musa::dnn::Tensor tensor_f8_scale_a;
    ::musa::dnn::Tensor tensor_f8_scale_b;
    ::musa::dnn::Tensor tensor_f8_scale_r;
    ::musa::dnn::Tensor tensor_golden_amax;
...
private:
    ...
    ::musa::dnn::Tensor::Type GetmuDNNType(const std::string& dtype) {
        ...
        static std::map<std::string, T> type_mapping = {
            ...
            {"bool", T::BOOL},
            {"float8_e4m3fn", T::FP8_E4M3},
            {"float8_e5m2", T::FP8_E5M2},
        };
    ...
    bool CheckParams() {
        ...
        if (dtype == DType::q8 || dtype == DType::q8) {
    ...
    bool Init() {
        ...
        if (dtype == DType::f16) {
            GenerateRandom<Eigen::half, float>((Eigen::half*)(h_buf_a), nr_elem_a, seed);
            GenerateRandom<Eigen::half, float>((Eigen::half*)(h_buf_b), nr_elem_b, seed);
            GenerateRandom<Eigen::half, float>((Eigen::half*)(h_buf_c), nr_elem_c, seed);
        }
        else if (dtype == DType::bf16) {
            GenerateRandom<Eigen::bfloat16, float>((Eigen::bfloat16*)(h_buf_a), nr_elem_a, seed);
            GenerateRandom<Eigen::bfloat16, float>((Eigen::bfloat16*)(h_buf_b), nr_elem_b, seed);
            GenerateRandom<Eigen::bfloat16, float>((Eigen::bfloat16*)(h_buf_c), nr_elem_c, seed);
        }
        ...
        if (gamma != 0) {
            ...
            if (dtype == DType::f16) {
                GenerateRandom<Eigen::half, float>((Eigen::half*)(h_buf_z), nr_elem_z, seed);
            }
            else if (dtype == DType::bf16) {
                GenerateRandom<Eigen::bfloat16, float>((Eigen::bfloat16*)(h_buf_z), nr_elem_z, seed);
            }
            else if (dtype == DType::q8) {
