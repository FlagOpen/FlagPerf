...
性能方面，以算子平均单次执行时间为基础，并扩展至吞吐、有效算力利用率等下游指标。执行时间以cpu墙钟时间以及内核执行时间为统计方式。详细指标内容及定义如下：
* 时延（Latency）指 Latency No Warm up 和 Latency Warm up，
Latency No Warm up 是第一个算子从开始执行到产生结果的时间，单
位为毫秒 ms。Latency Warm up 是完成 Warm up 后，第一个算子从开
始执行到产生结果的时间，单位为毫秒 ms。
* 吞吐率（Throughput）即较长时间内连续执行测试用例，计算
执行次数除以总时间，单位为 ops/s。
* Raw-Throughput 原始吞吐，即计算执行次数除以总时间。
* Core-Throughput 是核心吞吐，即计算执行次数除以核心时间。
* 实际算力开销即待测算子在运算时候实际利用的算力。
* 实际算力利用率即待测芯片实际利用的算力
