...
def check_config(config):
    ...
    if not train_data:
        train_data = ospath.join(data_dir,'ReCoRD/glm_train_eval_hdf5_sparse/train_hdf5/train_sparse.hdf5')
    ...
    if not eval_data:
        eval_data = ospath.join(data_dir,'ReCoRD/glm_train_eval_hdf5_sparse/eval_hdf5/eval_sparse.hdf5')
    ...
    if not init_checkpoint:
        init_checkpoint = ospath.join(data_dir,'blocklm-large-blank/200000/mp_rank_00_model_states.pt')
    ...
    print('config.gradient_accumulation_steps', config.gradient_accumulation_steps)
