...
class Trainer:
    ...
    def init(self):
        ...
        kw = {
            'benchmark_epochs': config.benchmark_epochs_num,
            'cuda': not config.cpu
        }
        self.metrics = Metrics(**kw)

        lr_kw = {
            'initial_lr_scale': config.initial_lr_scale,
        ...
        ema_model = self.init_train_config(config, self.train_dataloader)

        # for resume，default no save, no resume
        self.checkpointer = Checkpointer(config, 'wav2vec2')
    ...
    def train_all_epoch(self, config, epoch, step, train_dataloader, sampler):
        ...
        itr = islice(train_dataloader,
                     self.steps_per_epoch * config.update_freq)
        self.model.train()
        ...
        for batch, accum_batches in zip(itr, cycle(range(config.update_freq))):
            ...
            if (accum_batches + 1) % config.update_freq == 0:
                ...
                if config.optimizer == 'adam' and not (config.fp16
                                                       or config.bf16):
