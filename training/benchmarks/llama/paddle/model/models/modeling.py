...
from paddlenlp.transformers.conversion_utils import StateDictNameMapping, init_name_mappings
...
class LlamaRotaryEmbedding(nn.Layer):
    def __init__(self, dim, max_position_embeddings=2048, base=10000):
        ...
        # ic(t, self.inv_freq, self.inv_freq.cast("float32"))
...
class LlamaDecoderLayer(nn.Layer):
    # @profile(precision=4, stream=open("memory_profiler_train_decoder.log", "w+"))
    def __init__(self, config):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class LlamaModel(LlamaPretrainedModel):
    ...
    Args:
        config: LlamaConfig
    """
    # @profile(precision=4, stream=open("memory_profiler_train_llama.log", "w+"))
...
class LlamaPretrainingCriterion(paddle.nn.Layer):
    ...
    def forward(self, prediction_scores, masked_lm_labels):
        ...
        with paddle.amp.auto_cast(False):
            # ic(prediction_scores, masked_lm_labels)
...
class LlamaLMHead(nn.Layer):
    ...
    def forward(self, hidden_states, tensor_parallel_output=None):
        ...
        # ic(hidden_states, self.weight, tensor_parallel_output)
        return logits
