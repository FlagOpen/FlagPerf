...
from evaluator import Evaluator  # for end-of-epoch mAP
...
class Trainer:
    ...
    def train(self,
        ...
        # Model
        model = create_model(weights, cfg, resume, hyp, device)
        ...
        for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
            ...
            if RANK in [-1, 0]:
                ...
                if not noval or final_epoch:  # Calculate mAP
                    results, maps, _ = self.evaluator.evaluate(data_dict,
        ...
        if RANK in [-1, 0]:
            ...
            for f in last, best:
                if os.path.exists(f):
                    ...
                    if f is best:
                        LOGGER.info(f'\nValidating {f}...')
                        results, _, _ = self.evaluator.evaluate(data_dict,
                                                batch_size=batch_size // WORLD_SIZE * 2,
                                                imgsz=imgsz,
    ...
    def run(self, opt, training_state=TrainingState(), callbacks=Callbacks()): # flagperf
        # Checks
        if RANK in [-1, 0]:
        ...
        # Train
        if not opt.evolve:
            self.train(opt.hyp, opt, device, callbacks, training_state)
