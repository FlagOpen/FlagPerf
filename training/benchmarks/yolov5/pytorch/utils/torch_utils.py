...
def select_device(device='', batch_size=0, newline=True, n_device = 0):
    ...
    elif device and n_device:  # non-cpu device requested
        device = ",".join([str(i) for i in range(n_device)])
        os.environ['CUDA_VISIBLE_DEVICES'] = device  # set environment variable - must be before assert is_available()
        assert torch.cuda.is_available() , f"Invalid CUDA '--device {device}' requested, use '--device cpu' or pass valid CUDA device(s)"
