...
def kmean_anchors(dataset='./data/coco128.yaml',
    ...
    if isinstance(dataset, str):  # *.yaml file
        ...
        from dataloaders.dataloader import LoadImagesAndLabels
    ...
    if len(
            k
    ) != n:  # kmeans may return fewer points than requested if wh is insufficient or too similar
        LOGGER.warning(
            f'{PREFIX}WARNING: scipy.cluster.vq.kmeans returned only {len(k)} of {n} requested points'
        )
