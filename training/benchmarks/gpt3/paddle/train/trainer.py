...
class Trainer:
    ...
    def init(self):
        ...
        self.optimizer = self.adapter.create_optimizer(
            self.config, self.model, self.lr_scheduler
        )
        if self.config.fp16:
    ...
    def _init_model(self, model):
        checkpoint_path = os.path.join(
            self.config.base_path, self.config.data_dir, self.config.init_checkpoint
        )
    ...
    def train_one_epoch(self, dataloader, loss):
        ...
        for _, inputs in enumerate(dataloader):
            ...
            state.num_trained_samples = (
                state.global_steps * dist_paddle.global_batch_size(self.config)
            )
    ...
    def train_one_step(self, inputs):
        ...
        loss_step = self.adapter.backward(
            self.config,
            state.global_steps,
            loss_step,
            self.optimizer,
    ...
    def detect_training_status(self, state: TrainingState):
        if (
            state.global_steps >= self.config.max_steps
            or state.num_trained_samples >= self.config.max_samples_termination
        ):
