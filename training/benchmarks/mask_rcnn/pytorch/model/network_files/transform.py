...
def _resize_image_and_masks(
    ...
    if fixed_size is not None:
        ...
        scale = torch.min(self_min_size / min_size,
                          self_max_size / max_size)  # 计算缩放比例
...
def paste_mask_in_image(mask, box, im_h, im_w):
    ...
    mask = mask.expand(
        (1, 1, -1, -1))  # -1 means not changing the size of that dimension
...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def resize(self, image, target):
        ...
        image, target = _resize_image_and_masks(image, size,
    ...
    def _onnx_batch_images(self, images, size_divisible=32):
        ...
        for i in range(images[0].dim()):
            max_size_i = torch.max(
                torch.stack([img.shape[i] for img in images
                             ]).to(torch.float32)).to(torch.int64)
            max_size.append(max_size_i)
        stride = size_divisible
        max_size[1] = (torch.ceil(
            (max_size[1].to(torch.float32)) / stride) * stride).to(torch.int64)
        max_size[2] = (torch.ceil(
            (max_size[2].to(torch.float32)) / stride) * stride).to(torch.int64)
    ...
    def forward(
        ...
        images = self.batch_images(images,
                                   self.size_divisible)  # 将images打包成一个batch
