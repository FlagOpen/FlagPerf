...
class CompGraphConv(nn.Module):
    ...
    def forward(self, g, n_in_feats, r_feats):
        with g.local_scope():
            ...
            in_edges_idx = th.nonzero(g.edata["in_edges_mask"],
                                      as_tuple=False).squeeze()
            out_edges_idx = th.nonzero(g.edata["out_edges_mask"],
                                       as_tuple=False).squeeze()

            comp_h_O = self.W_O(comp_h[out_edges_idx])
            comp_h_I = self.W_I(comp_h[in_edges_idx])

            new_comp_h = th.zeros(comp_h.shape[0],
                                  self.out_dim).to(comp_h.device)
            ...
            g.update_all(fn.copy_e("new_comp_h", "m"),
                         fn.sum("m", "comp_edge"))
            ...
            n_out_feats = (self.W_S(comp_h_s) +
                           self.dropout(g.ndata["comp_edge"])) * (1 / 3)
...
class CompGCN(nn.Module):
    ...
    def __init__(
        ...
        if self.num_bases > 0:
            ...
            self.weights = nn.Parameter(th.Tensor(self.num_rel,
                                                  self.num_bases))
