...
class LogEventManager(EventManager):
    def __init__(self, logger: PerfLogger=None, log_freq: int = 0):
        ...
        self.logger = logger or PerfLogger.get_default_logger(rank=config.local_rank, level=level)

    def on_launch_training(self):
        self._log_event(Event.LAUNCH_TRAINING, "Launch training", stacklevel=STACKLEVEL)
