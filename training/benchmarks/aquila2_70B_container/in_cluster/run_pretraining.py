...
def merge_config(args):
    ...
    spec = spec_from_file_location("config", args.vendor_config)
...
if __name__ == "__main__":
    ...
    for ip in args.hosts:
        ...
        path_cmd = "cd " + os.path.join(args.perf_dir, "training/benchmarks/aquila2_7B_container/in_cluster")
        ...
        f = open(os.path.join(args.log_dir, "noderank" + str(noderank) + ".log.txt"), "a")
        exec_cmd = "mpirun -np " + str(config.NP) + " --hostfile " + str(config.HOSTFILE) + " singlenode_run.sh"
    ...
    world_size = int(config.NP)
    chip_throughput = system_throughput / world_size
    MFU = chip_throughput * 6 * 70000000000 / float(config.flops_16bit)
