...
class SavedModelCheckpointManager(tf.train.CheckpointManager):
    ...
    def save(self,
        ...
        """See base class."""
        checkpoint_path = super().save(checkpoint_number=checkpoint_number,
        ...
        saved_modules_directory = make_saved_modules_directory_name(
            checkpoint_path)
        for model_name, model in self._modules_to_export.items():
            ...
            tf.saved_model.save(obj=model,
                                export_dir=os.path.join(
                                    saved_modules_directory, model_name),
                                signatures=signatures)

        saved_modules_directories_to_keep = [
            make_saved_modules_directory_name(ckpt)
    ...
    def get_savedmodel_number_from_path(
            self, savedmodel_path: str) -> Union[int, None]:
    ...
    """
        ...
        while True:
            new_savedmodel_path = self.wait_for_new_savedmodel(savedmodel_path,
                                                               timeout=timeout)
    ...
    """
        ...
        while True:
            if stop_time is not None and time.time(
            ) + seconds_to_sleep > stop_time:
                return None
