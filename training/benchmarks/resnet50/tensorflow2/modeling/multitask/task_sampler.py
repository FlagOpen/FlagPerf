...
class TaskSampler(tf.Module, metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def task_cumulative_distribution(self,
...
class UniformTaskSampler(TaskSampler):
    ...
    def __init__(self, task_weights: Dict[Text, Union[float, int]]):
        ...
        self._uniform_cumulative = tf.math.cumsum(
            tf.constant([1.0 / len(self._task_weights)] *
                        len(self._task_weights),
                        dtype=tf.float32))
...
class ProportionalTaskSampler(TaskSampler):
    ...
    def __init__(self,
                 ...
                 alpha: float = 1.0):
        super(ProportionalTaskSampler,
              self).__init__(task_weights=task_weights)
...
class AnnealingTaskSampler(TaskSampler):
  ...
  """
    ...
    def __init__(self, task_weights: Dict[Text, Union[float, int]],
        ...
        self._total_epochs = tf.cast(total_steps / self._steps_per_epoch,
                                     dtype=tf.float32)
...
def get_task_sampler(config: configs.TaskSamplingConfig,
    ...
    if config.type == 'uniform':
        ...
        return ProportionalTaskSampler(task_weights=task_weights,
                                       alpha=oneof_config.alpha)
