  ...
  """
    ...
    def update_average(self, step: tf.Tensor):
        ...
        if step < self._start_step:
            ...
            decay = tf.minimum(self._average_decay,
                               (1. + decay) / (10. + decay))
        else:
        ...
        def _update(strategy, v_moving_and_v_normal):
            for v_moving, v_normal in v_moving_and_v_normal:
                strategy.extended.update(v_moving,
                                         _apply_moving,
                                         args=(v_normal, ))

        ctx = tf.distribute.get_replica_context()
        return ctx.merge_call(_update,
                              args=(zip(self._average_weights,
                                        self._model_weights), ))
