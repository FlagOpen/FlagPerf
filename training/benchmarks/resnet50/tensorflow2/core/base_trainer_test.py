...
def dataset_fn(input_context=None):
    ...
    dataset = dataset.map(dummy_data,
...
class MockAsyncTrainer(trainer_lib._AsyncTrainer):
    ...
    def __init__(self):
        ...
        orbit.StandardTrainer.__init__(self,
                                       train_dataset,
...
class TrainerTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def create_test_trainer(self, config, model_dir=None, task=None):
        ...
        ckpt_exporter = train_lib.maybe_create_best_ckpt_exporter(
            config, model_dir)
    ...
    def test_trainer_passing_datasets(self, distribution):
        with distribution.scope():
            ...
            validation_dataset = orbit.utils.make_distributed_dataset(
                distribution, task.build_inputs,
                self._config.task.validation_data)
            ...
            trainer = trainer_lib.Trainer(
                ...
                model=task.build_model(),
                optimizer=task.create_optimizer(
                    self._config.trainer.optimizer_config,
