...
class _AsyncTrainer(orbit.StandardTrainer, orbit.StandardEvaluator):
    ...
    def create_train_loop_fn(self):
        ...
        if getattr(self, "_is_async", False):
            ...
            def _async_loop_fn(iterator, num_steps):
                self._coordinator.schedule(train_loop_fn,
                                           args=(iterator, num_steps))
    ...
    def create_eval_loop_fn(self, has_state: bool):
        ...
        if getattr(self, "_is_async", False):
            ...
            def _async_loop_fn(iterator,
                               num_steps,
                               state=None,
                               reduce_fn=None):
                assert state is None
                assert reduce_fn is None
                self._coordinator.schedule(eval_loop_fn,
                                           args=(iterator, num_steps))
    ...
    """
        if getattr(self, "_is_async", False):
            per_worker_dataset_fn = functools.partial(
                orbit.utils.make_distributed_dataset, self._strategy,
                dataset_or_fn, *args, **kwargs)
            per_worker_dataset_fn = tf.function(per_worker_dataset_fn)

            return self._coordinator.create_per_worker_dataset(
...
def get_runtime_options(config: ExperimentConfig):
    ...
    return tf.distribute.RunOptions(experimental_xla_options=tf.tpu.XLAOptions(
        **xla_options))
...
class Trainer(_AsyncTrainer):
    ...
    """
        ...
        if check_validation_data and not hasattr(config.task,
                                                 "validation_data"):
            raise AttributeError(
                "The trainer requires the configuration contains an"
    ...
    def train_step(self, iterator):
        ...
        def step_fn(inputs):
            if self.config.runtime.enable_xla and (self.config.runtime.num_gpus
                                                   > 0):
    ...
    def eval_step(self, iterator):
        ...
        def step_fn(inputs):
            logs = self.task.validation_step(inputs,
                                             model=self.model,
        ...
        distributed_outputs = self.strategy.run(step_fn,
                                                args=(next(iterator), ))
    ...
    def eval_end(self, aggregated_logs=None):
        ...
        if self._checkpoint_exporter:
            ...
            logs["best_" +
                 metric_name] = self._checkpoint_exporter.best_ckpt_logs[
                     metric_name]
