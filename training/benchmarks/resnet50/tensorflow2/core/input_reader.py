...
def _shard_files_then_read(matched_files: List[str],
    ...
    dataset = dataset.interleave(
        ...
        num_parallel_calls=(cycle_length if cycle_length else
...
class InputReader:
    ...
    """
        ...
        if (isinstance(params.input_path, cfg.base_config.Config)
                or isinstance(params.tfds_name,
    ...
    def _read_data_source(
        ...
        def _files_to_dataset(files: List[str]) -> tf.data.Dataset:
            if len(files) > 1:
                if input_context and (len(files) <
                    ...
                    logging.warn(
                        ...
                        len(files), input_context.num_input_pipelines)
                    return _read_files_then_shard(files,
                                                  dataset_fn,
                                                  input_context,
                                                  sharding=self._sharding,
                                                  repeat=self._is_training
                                                  and not self._cache)
                else:
            ...
            if isinstance(tfds_builder, dict):
                ...
                for k, builder in tfds_builder.items():
                    dataset[k] = _read_tfds(
                        ...
                        tfds_skip_decoding_feature=self.
    ...
    def _decode_and_parse_dataset(
        ...
        if (self._enable_shared_tf_data_service_between_parallel_trainers
                and self._apply_tf_data_service_before_batching):
    ...
    def read(self,
        ...
        if dataset is None:
            dataset = self._read_data_source(self._matched_files,
                                             self._dataset_fn, input_context,
                                             self._tfds_builder)
        dataset = self._decode_and_parse_dataset(dataset,
                                                 self._global_batch_size,
                                                 input_context)
        dataset = _maybe_map_fn(dataset, self._postprocess_fn)
        if not (self._enable_shared_tf_data_service_between_parallel_trainers
                and self._apply_tf_data_service_before_batching):
            dataset = self._maybe_apply_data_service(dataset, input_context)
