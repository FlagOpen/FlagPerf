...
class ExpFactoryInfo(function_page.FunctionPageInfo):
    ...
    def make_factory_options_table(self):
        ...
        for name, fn in sorted(
                tfm.core.exp_factory._REGISTERED_CONFIGS.items()):  # pylint: disable=protected-access
            fn_api_node = api_tree.node_for_object(fn)
            if fn_api_node is None:
                location = parser.get_defined_in(self.py_object,
                                                 self.parser_config)
                link = base_page.small_source_link(location, name)
            else:
                link = reference_resolver.python_link(name,
                                                      fn_api_node.full_name)
            doc = fn.__doc__
            if doc:
...
def custom_filter(path, parent, children):
    if len(path) <= 2:
        ...
        return public_api.explicit_package_contents_filter(
...
def gen_api_docs(code_url_prefix, site_path, output_dir, project_short_name,
    ...
    doc_controls.set_custom_page_builder_cls(
        tfm.core.exp_factory.get_exp_config, ExpFactoryInfo)
