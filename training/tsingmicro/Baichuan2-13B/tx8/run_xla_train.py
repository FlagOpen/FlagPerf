...
# PrecisionDebuggerINIT(config_path="/login_home/zhangna/transformers-4.46.1_2048_577/examples/train/Baichuan2-13B/tx8/config_tensor.json")
...
class ModelArguments:
    ...
    mstt_config_name_or_path: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "debug mstt config path."
            )
        },
    ) 
    tsprobe_config: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "tsprobe config file path."
            )
        },
    )
    ...
    fsdp_dp_sharding: Optional[int] = field(
        default=-1,
        metadata={
            "help": (
                "FSDP dp number."
    ...
    megatron_tp_sharding: Optional[int] = field(
        default=1,
        metadata={
            "help": (
                "megatron tp number."
    ...
    use_flash_attn: bool = field(
        default=False, metadata={"help": "Whether to use FlashAttention."}
    )
...
def main():
    ...
    if not model_args.use_cuda:
        tx8_replace_func .replace_tx8_llama_rotary_embedding()
    ...
    if not model_args.use_cuda:
        if model_args.use_flash_attn:
            ...
            model = tx8_replace_func.replace_BaichuanModel_forward(model)
        else:
            model = tx8_replace_func.replace_BaichuanAttention_forward(model)
        tx8_replace_func.replace_tx8_mask()
        model = tx8_replace_func.replace_tx8_rmsNorm(model)
        # tx8_replace_func.replace_tx8_rope(config)
        tx8_replace_func.replace_TrainingArguments_setup_devices()
        tx8_replace_func.replace_torch_tril_gt_ge_eq_ne_lt_le()
    ...
    if is_fsdp_xla_enabled and not model_args.use_cuda:
        ...
        fsdp_wrap = lambda m: FSDP(m,auto_wrap_policy=auto_wrap_policy,auto_wrapper_callable=auto_wrapper_callable,**fsdp_kwargs,)
        import inspect
...
def _mp_device_preprocess(global_rank, group_rank, local_rank):
