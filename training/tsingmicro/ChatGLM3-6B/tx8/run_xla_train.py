...
def preprocess(
    ...
    if input_ids.size(1) < max_len:
        padding_length = max_len - input_ids.size(1)
        input_ids = torch.nn.functional.pad(input_ids, (0, padding_length), value=tokenizer.pad_token_id)
    target_ids = input_ids.clone()
    target_ids[target_ids == tokenizer.pad_token_id] = IGNORE_TOKEN_ID
    attention_mask = input_ids.ne(tokenizer.pad_token_id).to(torch.bfloat16)
...
def main(mg_semap=None):
    ...
    # chagne layer number
    config.seq_length = data_args.block_size
    config.num_layers = model_args.num_hidden_layers
    tokenizer_kwargs = {
        "cache_dir": model_args.cache_dir,
    ...
    if not model_args.use_cuda:
        if torch_xla._XLAC.IsCurrentDeviceTx8():
                from tx8_replace_func import (
                    replace_coreattention, 
                    replace_unsupportcall, 
                    replace_rope_embedding, 
                    replace_customcall,
                    replace_self_attention_forward,
                    gen_new_weight_order
                    )
                model = replace_customcall(model)
                model = replace_coreattention(model)
                model = replace_unsupportcall(model)
                model = replace_rope_embedding(model)
                model = replace_self_attention_forward(model)
                new_order = gen_new_weight_order(config)
                for layer in model.transformer.encoder.layers:
                    layer.self_attention.query_key_value.weight.data = layer.self_attention.query_key_value.weight.data[new_order, :]
                    layer.self_attention.query_key_value.bias.data = layer.self_attention.query_key_value.bias.data[new_order]

    if model_args.megatron_tp_sharding > 1:
        from transformers.trainer_pt_utils import get_module_class_from_name
        from torch_xla.distributed.fsdp.wrap import size_based_auto_wrap_policy,transformer_auto_wrap_policy
        from torch_xla.distributed.megatron_dtp.dtp import TX8DataMegatronParallel,ModelType
        import torch_xla.distributed.megatron_dtp.initialize as initialize
        for layer in model.transformer.encoder.layers:
            layer.self_attention.num_attention_heads_per_partition = layer.self_attention.num_attention_heads_per_partition // model_args.megatron_tp_sharding
            layer.self_attention.num_multi_query_groups_per_partition = layer.self_attention.num_multi_query_groups_per_partition // model_args.megatron_tp_sharding
            layer.self_attention.core_attention.hidden_size_per_partition = layer.self_attention.core_attention.hidden_size_per_partition // model_args.megatron_tp_sharding

        ignored_modules = []
        for name, child in model.named_modules():
            if name == 'transformer.output_layer':
                ignored_modules.append(child)
        initialize.initialize_model_parallel(model_args.megatron_tp_sharding, model_args.ddp_dp_sharding)
        transformer_cls = get_module_class_from_name(model, "Linear")
        auto_wrap_policy = functools.partial(transformer_auto_wrap_policy, transformer_layer_cls=set([transformer_cls]))
        model = TX8DataMegatronParallel(model, row_parallel_groups=["dense","dense_4h_to_h"], ignored_modules=ignored_modules,
            col_parallel_groups=["query_key_value","dense_h_to_4h"],auto_wrap_policy=auto_wrap_policy ,megatron_tp_model_type=ModelType.ChatGLM)
...
def _mp_fn(local_rank, mg_semap):
    ...
    print(f"!!!!!!rank:{local_rank},CUDA_VISIBLE_DEVICES: {os.environ['CUDA_VISIBLE_DEVICES']}")
