...
class XlaTrainer:
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        total_rank = self.model_args.fsdp_dp_sharding * self.model_args.megatron_tp_sharding
        total_tensor = torch.tensor(range(total_rank))

        if self.model_args.use_cuda:
            all_reduce_sharding_groups = []
            for i in range(self.model_args.fsdp_dp_sharding):
            ...
            dp_tensor = total_tensor.view(-1, self.model_args.fsdp_dp_sharding).transpose(0, 1)
            all_reduce_sharding_groups = dp_tensor.tolist() if not hasattr(self.model_args,'all_reduce_sharding_groups') else self.model_args.all_reduce_sharding_groups
    ...
    def _prepare_inputs(self, inputs: Dict[str, Union[torch.Tensor, Any]]) -> Dict[str, Union[torch.Tensor, Any]]:
        ...
        label = inputs['labels']
        inputs['labels'] = label.to(torch.int32)
        return inputs
    ...
    def train(
        ...
        **kwargs,
        ...
        if self.model_args.use_dynamic_shape:
            tr_loss = 0
            for decoder_layer in self.model._fsdp_wrapped_module._fpw_module.model.layers:
                decoder_layer._fsdp_wrapped_module._fpw_module.block_sparse_moe.dynamic_shape = True
            max_len = self.train_dataset.max_len
            per_device_train_batch_size = self.args.per_device_train_batch_size
            input_ids = torch.full((per_device_train_batch_size, max_len), 128)
            labels = torch.full((per_device_train_batch_size, max_len), 128)
            attention_mask = torch.full((per_device_train_batch_size, max_len), True, dtype=torch.bool)
            inputs = {"input_ids":input_ids, "labels":labels, "attention_mask":attention_mask}
            ...
            if not self.model_args.use_cuda:
                xm.mark_step()
            for decoder_layer in self.model._fsdp_wrapped_module._fpw_module.model.layers:
                decoder_layer._fsdp_wrapped_module._fpw_module.block_sparse_moe.dynamic_shape = False
