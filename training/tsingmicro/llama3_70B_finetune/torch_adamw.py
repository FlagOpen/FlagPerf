        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                with torch.inference_mode():
                    eps_tensor = torch.tensor(group["eps"], dtype=p.dtype)
                    beta1_tensor = torch.tensor(beta1, dtype=p.dtype)
                break
...
def _single_tensor_adamw(
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        with torch.inference_mode():
            beta2_tensor = torch.tensor((1-beta2), dtype=grad.dtype)
        # exp_avg_sq.mul_(beta2).addcmul_(grad, grad, value=1 - beta2)
        exp_avg_sq.mul_(beta2).add_(beta2_tensor * grad * grad)
        ...
        if capturable or differentiable:
            ...
            if amsgrad:
                ...
                #denom = (exp_avg_sq.sqrt() / (bias_correction2_sqrt * step_size + eps)).add_(eps)
                denom = (exp_avg_sq.sqrt() / (bias_correction2_sqrt * step_size + eps))
                ft32_denon = denom.to(torch.float32)
                denom = ft32_denon.add_(eps)
                denom = denom.to(exp_avg.dtype)
