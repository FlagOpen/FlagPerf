...
def main(mg_semap=None):
    ...
    if not model_args.use_cuda:
        tx8_replace_llama.replace_tx8_llama_rotary_embedding()
    ...
    if not model_args.use_cuda:
        if model_args.use_flash_attn:
            model = tx8_replace_llama.enable_tx8_flash_attention(model)
        tx8_replace_llama.replace_tx8_mask()
        model = tx8_replace_llama.replace_tx8_rmsNorm(model)
        tx8_replace_llama.replace_tx8_rope(config)
        tx8_replace_llama.replace_TrainingArguments_setup_devices()
    ...
    if is_fsdp_xla_enabled and not model_args.use_cuda:
        ...
        if model_args.fsdp_dp_sharding > 1  and model_args.megatron_tp_sharding > 1:
            ...
            tx8_replace_llama.replace_tx8_llama_decode_forward()
            ...
            fsdp_tp_kwargs["ignore_params"] = ["input_layernorm","post_attention_layernorm"]
            '''
            '''
            #8cards 4fsdp+2tp
        ...
        import inspect
        forward_signature = inspect.signature(model.forward.__func__)
        model = FSDP(model, auto_wrap_policy=auto_wrap_policy, device_mesh=dp_mesh, device_id=dist.get_rank(),
