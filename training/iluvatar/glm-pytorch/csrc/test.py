...
args = [[10, 512, 1024], [20, 512, 1024], [
    32, 512, 1024], [64, 512, 1024], [80, 512, 1024]]
...
def gen_inputs(batch_size, seq_len, hidden_size):
    ...
    inputs1 = torch.tensor(data, dtype=torch.float16,
                           device=device, requires_grad=True)
    inputs2 = torch.tensor(data, dtype=torch.float16,
                           device=device, requires_grad=True)

    mask_data = np.random.randint(0, seq_len, batch_size)

    mask_1 = torch.tensor(mask_data, dtype=torch.int64,
                          device=device, requires_grad=False)
    mask_2 = torch.tensor(mask_data, dtype=torch.int64,
                          device=device, requires_grad=False)

    mask_1 = build_mask_matrix(inputs1, seq_len, mask_1).byte() # batch_size*num_heads,seq_len,seq_len
...
if __name__ == "__main__":
    layer1 = SelfMultiheadAttn(hidden_size, num_attention_heads,bias=True, impl='default').to(device).half()
    layer2 = SelfMultiheadAttn(hidden_size, num_attention_heads,bias=True, impl='fast').to(device).half()
    ...
    for case_arg in args:
        ...
        input_args_1 = {"query":inputs1, "attn_mask":mask1, "is_training":True}
        input_args_2 = {"query":inputs2, "attn_mask":mask2, "is_training":True}
        ...
        forward_test_results[case_str]['correct_rate'] = (
            diff < forward_pass_threshold).sum().cpu().item()/torch.numel(diff)
        ...
        backward_test_results[case_str]['correct_rate'] = (
            diff < backward_pass_threshold).sum().cpu().item()/torch.numel(diff)
        ...
        def compute_impl_time(time_results):
            return (sum(time_results) - max(time_results)) / (len(time_results) - 1)

        forward_test_results[case_str]['native_impl_time'] = compute_impl_time(
            perf_results_1)
        forward_test_results[case_str]['speedup_impl_time'] = compute_impl_time(
            perf_results_2)
        forward_test_results[case_str]['speedup_ratio'] = compute_impl_time(
            perf_results_1) / compute_impl_time(perf_results_2)

        backward_test_results[case_str]['native_impl_time'] = compute_impl_time(
            bk_perf_results_1)
        backward_test_results[case_str]['speedup_impl_time'] = compute_impl_time(
            bk_perf_results_2)
        backward_test_results[case_str]['speedup_ratio'] = compute_impl_time(
            bk_perf_results_1) / compute_impl_time(bk_perf_results_2)

    print("### forward ")
    print(f"| No. | parameters | native_impl_time | speedup_impl_time | speedup_ratio | correctness | correct_rate |")
    print('| --- | --- | --- | --- | --- | --- | --- |')
