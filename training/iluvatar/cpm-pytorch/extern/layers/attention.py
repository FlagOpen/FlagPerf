...
class OfficialSelfAttention(torch.nn.Module):
    def __init__(self, hidden_size, num_attention_heads,
        ...
        self.layer = MultiheadAttention(embed_dim=hidden_size,
    ...
    def forward(self, hidden_states, ltor_mask, *args, **kwargs):
        ...
        output = self.layer(hidden_states, hidden_states,
                            hidden_states, attn_mask=mask, need_weights=False)
        output = output[0]
...
class OfficialSelfAttentionApex(torch.nn.Module):
    def __init__(self, hidden_size, num_attention_heads,
        ...
        self.num_attention_heads = num_attention_heads
        self.layer = SelfMultiheadAttn(embed_dim=hidden_size,
                                       num_heads=num_attention_heads,
                                       dropout=attention_dropout_prob,
                                       bias=True,
    ...
    def forward(self, hidden_states, ltor_mask, *args, **kwargs):
        ...
        mask = mask.repeat([1, self.num_attention_heads, 1, 1])
        mask = mask.view([-1, mask.shape[-2], mask.shape[-1]])
        output = self.layer(hidden_states, attn_mask=mask, is_training=self.training)
        output = output.transpose(0, 1)
        output = self.output_dropout(output)
        return output
