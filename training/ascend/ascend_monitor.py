...
class Daemon:
    ...
    def run(self):
        ...
        def timer_npu_mon():
            npu_process = Process(target=npu_mon, args=(self.npufile, ))
            npu_process.start()

        schedule.every(self.rate).seconds.do(timer_npu_mon)
        while True:
    ...
    def start(self):
        ...
        elif os.path.exists(self.npufile):
            os.remove(self.npufile)
        if self.verbose >= 1:
...
def get_system_info():
    ...
    cmd = cmd + r"npu-smi info -l;"
    cmd = cmd + r"echo ;"
    
    cmd = cmd + r"echo Accelerator Driver version:;"
    cmd = cmd + r"npu-smi info | grep 'Driver Version' | awk '{print $3}';"
    cmd = cmd + r"echo ;"
