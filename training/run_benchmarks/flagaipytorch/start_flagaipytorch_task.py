...
def parse_args():
    ...
    parser = ArgumentParser(
        description="Start flagai pytorch training porcess. ")
...
def _set_common_ddp_envs(task_args):
    ...
    current_env["N_DEVICE"] = str(task_args.nproc)
...
def _get_basic_train_script_args(task_args):
    ...
    basic_train_script_args += " --not_call_launch"
...
def main():
    ...
    task_args.framework = "flagaipytorch"

    task_log_dir = helper.init_flagperf_logger(START_LOGGER, task_args)
    helper.write_pid_file(task_args.log_dir, "start_flagaipytorch_task.pid")
