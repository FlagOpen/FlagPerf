...
def parse_args():
    '''we parse ddp related args, check system config args, and running env
       ...
       training script.
    '''
    parser = ArgumentParser(description="Start mindspore training process. ")
...
def _set_common_ddp_envs(task_args):
    ...
    # set mindspore distributed related environmental variables
    ...
    # set NPU/MLU device env, TODO other vendor's device
...
def main():
    ...
    processes = subprocess.Popen(start_cmd, shell=True, env=current_env)
    processes.wait()
