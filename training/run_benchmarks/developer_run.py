...
def prepare_running_env(dp_path, container_name, case_config, f, should_sout):
    ...
    sys.stdout = should_sout
    result_cmd = "python3 " + tc.FLAGPERF_PATH_CONTAINER + "/" \
                  + "/run_benchmarks/prepare_in_container.py --framework " \
                  + framework + " --model " + model + " --vendor " \
                  + tc.VENDOR + " --pipsource " + tc.PIP_SOURCE
    print("Command 2, run at host:")
    r = "docker exec -it " + container_name + " /bin/bash"
    print(r)
    print("Command 3, run at container:")
    print(result_cmd)
    sys.stdout = f
    timeout = 1800
    RUN_LOGGER.debug(
...
def stop_container_in_cluster(dp_path, container_name, nnodes, f, should_sout):
    ...
    RUN_LOGGER.debug("Run cmd to stop container(s) in the cluster:" +
                     stop_cont_cmd)
    sys.stdout = should_sout
    foo = input("PRESS ANY KEY TO STOP CONTAINER " + container_name)
    sys.stdout = f
...
def start_tasks_in_cluster(dp_path, container_name, case_config, base_args,
    ...
    result_cmd = "source " + env_file + "; " \
                + "python3 " + tc.FLAGPERF_PATH_CONTAINER + "/run_benchmarks/" \
                + framework + "/start_" + framework + "_task.py " \
                + base_args + " --round " + str(count)
    if tc.ACCE_VISIBLE_DEVICE_ENV_NAME is not None:
        result_cmd += " --visible_dev_env " \
                     + tc.ACCE_VISIBLE_DEVICE_ENV_NAME
    result_cmd = result_cmd + ' --master_addr ' + CLUSTER_MGR.hosts[0]
    for i in range(0, nnodes):
        host = CLUSTER_MGR.hosts[i]
        result_cmd = result_cmd + ' --node_rank ' + str(
            i) + ' --host_addr ' + host + "\""
    sys.stdout = should_sout
    print("Command 4, run at container:")
    print(result_cmd)
    print(
        "The above commands are generated by files such as test_conf, commands 1-3 have been automatically executed by "
        "flagperf, you can execute command 4 inside the container to start the model, or execute similar commands for "
        "debugging")
    sys.stdout = f
    exit(0)
    if tc.ACCE_VISIBLE_DEVICE_ENV_NAME is not None:
...
def prepare_containers_env_cluster(dp_path, case_log_dir, container_name,
                                   image_name, case_config, f, should_sout):
    ...
    stop_container_in_cluster(dp_path, container_name, nnodes, f, should_sout)
    RUN_LOGGER.info("b) Start container(s) in the cluster.")
    r = "docker run " + container_start_args
    r = r + " --name=" + container_name
    r = r + " \"" + image_name + "\"" + " sleep infinity"
    sys.stdout = should_sout
    ...
    if not prepare_running_env(dp_path, container_name, case_config, f,
                               should_sout):
