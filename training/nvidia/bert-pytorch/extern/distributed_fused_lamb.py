...
def _pipeline_block_reductions_patched(self, block_id):
    ...
    for chunk_id in range(self._num_chunks):
        ...
        with torch.cuda.stream(rs_stream):
            works[chunk_id] = torch.distributed.reduce_scatter(self._fp16_g_chunks[block_id][chunk_id],self._flat_grads_shards[block_id][chunk_id],group=self._rs_pg[glob_chunk_id%self._num_rs_pg],async_op=True, **no_copy_args)
...
def _pipeline_step_patched(self):
    ...
    with torch.cuda.stream(self._completion_st):
        ...
        multi_tensor_applier(self.multi_tensor_lamb_update_weights,
                ...
                global_grad_norm,
                self._use_nvlamb)
        torch.distributed.all_gather(self._new_params_mega_shards, self._fp16_p, group=self._ag_pg[0], **no_copy_args)
