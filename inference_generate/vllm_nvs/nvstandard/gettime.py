...
def time_monitor(logfile,stop_event):
    ...
    model=vllm.LLM(model=local_model_path,tensor_parallel_size=1)
    ...
    for i in tqdm(range(10),desc="inference"):
        with torch.no_grad():
            ...
            inputs =input_text+"Please generate a summary about it:"
            ...
            #     top_k=50
            # )
            outputs=model.generate(inputs, **params1)
            ...
            #     top_k=50
            # )
            outputs=model.generate(inputs, **params2)
